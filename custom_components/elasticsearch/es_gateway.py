...
class ConnectionMonitor:
    ...
    async def async_init(self):
        ...
        self.task = self.gateway.config_entry.async_create_background_task(
            self.gateway.hass,
    ...
    async def stop(self):
        ...
        if not self.active:
            LOGGER.debug("There was no active connection to [%s].", self.gateway.url)
...
class ElasticsearchGateway(ABC):
    ...
    def __init__(
        ...
        """Non-I/O bound init."""
        # Lift common attributes to this class
        self._hass = hass
        self._client = None
    ...
    async def async_init(self):
        ...
        self.client = self._create_es_client(self._client_args)
    ...
    async def _test_connection(self, **kwargs):
        ...
        LOGGER.debug("Testing the connection for [%s].", url)

        try:
            # Create an Elasticsearch client
            await self._create_es_client(**kwargs).async_init()
