...
def mock_es_initialization(
    ...
    if mock_serverless_version:
        aioclient_mock.get(
            url, headers=headers, status=200, json=CLUSTER_INFO_SERVERLESS_RESPONSE_BODY
        )
        ...
        aioclient_mock.get(
            ...
            headers=headers,
            status=200,
            json=CLUSTER_INFO_UNSUPPORTED_RESPONSE_BODY,
        )
    elif mock_authentication_error:
        aioclient_mock.get(
            url, headers=headers, status=401, json={"error": "unauthorized"}
        )
    elif mock_connection_error:
        aioclient_mock.get(url, headers=headers, status=500, json={"error": "idk"})
    elif mock_v811_cluster:
        aioclient_mock.get(
            url, headers=headers, status=200, json=CLUSTER_INFO_8DOT11_RESPONSE_BODY
        )
    elif mock_v88_cluster:
        aioclient_mock.get(
            url, headers=headers, status=200, json=CLUSTER_INFO_8DOT8_RESPONSE_BODY
        )
    elif mock_v80_cluster:
        aioclient_mock.get(
            url, headers=headers, status=200, json=CLUSTER_INFO_8DOT0_RESPONSE_BODY
        )
    elif mock_v711_cluster:
        aioclient_mock.get(
            url, headers=headers, status=200, json=CLUSTER_INFO_7DOT11_RESPONSE_BODY
        )
    elif mock_v717_cluster:
        aioclient_mock.get(
            url, headers=headers, status=200, json=CLUSTER_INFO_7DOT17_RESPONSE_BODY
        )
    else:
        aioclient_mock.get(
            url, headers=headers, status=200, json=CLUSTER_INFO_RESPONSE_BODY
        )
