...
async def test_entity_detail_publishing(hass, es_aioclient_mock: AiohttpClientMocker, mock_config_entry: mock_config_entry):
    ...
    entity_registry.async_get(hass).async_update_entity(entity_id, area_id=entity_area.id, device_id=device.id, name=entity_name)
...
async def test_include_exclude_publishing_mode_all(hass: HomeAssistant, es_aioclient_mock: AiohttpClientMocker):
    ...
    input_boolean_config = {input_boolean.DOMAIN: {"test_1": { "name": "test boolean 1", "initial": False}, "test_2": {"name": "test boolean 2", "initial": True}}}
    ...
    input_text_config = {input_text.DOMAIN: {"test_1": { "name": "test text 1", "initial": 'Hello'}, "test_2": {"name": "test text 2", "initial": 'World'}}}
...
async def test_publish_modes(hass: HomeAssistant, es_aioclient_mock: AiohttpClientMocker, publish_mode):
