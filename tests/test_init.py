...
class Test_Common_e2e:
    ...
    async def test_setup_to_skip_publish(
        self,
        hass: HomeAssistant,
        integration_setup,
        es_aioclient_mock: AiohttpClientMocker,
        ...
        config_entry.runtime_data._pipeline_manager._gateway.check_connection = mock.Mock(return_value=False)
        config_entry.runtime_data._pipeline_manager._sip_queue = mock.Mock()

        # Manually invoke a publish
        await config_entry.runtime_data._pipeline_manager._publish()

        # Our gateway is offline so assert we did not attempt to sip the queue to publish data
        assert es_aioclient_mock.call_count == 0

        config_entry.runtime_data._pipeline_manager._sip_queue.assert_not_called()

    async def test_setup_to_publish_404_error(
        self,
        hass: HomeAssistant,
        integration_setup,
        es_aioclient_mock: AiohttpClientMocker,
        ...
        config_entry.runtime_data._pipeline_manager._logger.debug.assert_called_with(
            "Connection error in publishing loop. Reloading integration.",
            exc_info=True,
        )
    ...
    async def test_setup_to_publish_403_error(
        self,
        hass: HomeAssistant,
        integration_setup,
        es_aioclient_mock: AiohttpClientMocker,
        ...
        config_entry.runtime_data._pipeline_manager._logger.debug.assert_called_with(
            "Authentication issue in publishing loop. Reloading integration.",
            exc_info=True,
        )
